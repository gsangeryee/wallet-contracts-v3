...
This report claims that per-call signatures can be extracted from a multi-call payload and replayed independently. Each call is individually signed with hashCallWithReplayProtection(payload, i), allowing partial signature extraction.
Although Sequence's per-call signature design allows individual calls to be replayed across different payloads. While session mixing within a payload is intended, cross-payload replay of individual calls was not. As a result, intercepted signed calls can be reused in different payload contexts, bypassing intended execution semantics.

This vector could be used to cause direct fund loss. Take the standard Uniswap LP addition operation (also quite commonly used):

User's batched session transaction:
[
tokenA.transfer(pairAddress, 1000e18), // Step 1: Send token A
tokenB.transfer(pairAddress, 1000e18), // Step 2: Send token B
pair.mint(userAddress) // Step 3: Mint LP tokens
]
If the attacker extracts partial signatures: [transfer(tokenA), transfer(tokenB)] and frontrun, the victim will lose the token A and B, but cannot receive the LP.

Per C4 criteria:

Direct asset loss
Guaranteed exploit
This report fits high severity.
